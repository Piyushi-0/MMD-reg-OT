...
from ot_mmd.utils import get_G, createLogHandler
    ...
    with torch.no_grad():
        with open(out_path, 'w') as f:
            for i in range(len(loader['target_label'])):
                inputs, labels, paths = next(iter_label) #.next()
                inputs = inputs.cuda()
...
def get_id_K(y):
    return (y[:, None]==y).to(torch.float)
    ...
    for i in tqdm(range(config["num_iterations"]), total=config["num_iterations"]):
        if i % config["test_interval"] == config["test_interval"]-1:
            ...
            if temp_acc > best_acc:
                ...
                print("\n##########     save the best model.    #############\n")
            # log_str = "iter: {:05d}, temp_acc: {:.5f}".format(i, temp_acc)
            # config["out_file"].write(log_str+"\n")
            # config["out_file"].flush()
        ...
        xs, ys = next(iter_source) #.next()  # source minibatch
        xt, _ = next(iter_target) #.next()  # target minibatch
        ...
        det_g_xt = g_xt.detach()
        
        G1 = get_G(ktype=config["ktype"], khp=config["khp"], x=g_xs.detach(), y=g_xs.detach())*get_id_K(ys.argmax(dim=1))
        G2 = get_G(ktype=config["ktype"], khp=config["khp"], x=det_g_xt, y=det_g_xt)*get_id_K(det_g_xt.argmax(dim=1))
        G = {1: G1, 2: G2}
        pi, _ = solve_apgd(M.detach(), G, {1: a, 2: b}, 200, config["lda"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--khp', type=float)
    parser.add_argument('--ktype', type=str)
    parser.add_argument('--out_file', type=str, default="logs")
    ...
    logger = createLogHandler(args.out_file+".csv", str(os.getpid()))
    ...
    if config["dataset"] == "office":
        ...
        config["optimizer"]["lr_param"]["lr"] = args.lr  # NOTE: changed from 0.001
    ...
    results = []
    config["alpha"] = args.alpha  # NOTE: changed  from 0.01
    config["lambda_t"] = args.lambda_reg  # NOTE: changed  from 0.5
    config["lda"] = args.lda
    config["ktype"] = args.ktype
    config["khp"] = args.khp
    
    results.append(train(config))
    logger.info(f', {config["optimizer"]["lr_param"]["lr"]}, {config["alpha"]}, {config["lambda_t"]}, {config["lda"]}, \
                    {config["ktype"]}, {config["khp"]}, {results[-1]}')
